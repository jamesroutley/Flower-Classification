function [train_instance_matrix, test_instance_matrix, ...
    train_label_vector, test_label_vector] = investigate_accuracy_cnn ...
...
if train_mirror == 1 && train_jitter == 1
    train_instance_matrix = zeros(17 * 12, 4096);
    train_label_vector = zeros(17 * 12, 1);
    for i = 1 : 17
        train_instance_matrix(12*i - 11, :) = instance_matrix_standard(trainid(((i-1)*40 + 1)), :);
        train_instance_matrix(12*i - 10, :) = instance_matrix_mirror(trainid(((i-1)*40 + 1)), :);
        train_instance_matrix(12*i-9 : 12*i-5, :) = instance_matrix_jitter(5*trainid(((i-1)*40 + 1))-4 : 5*trainid(((i-1)*40 + 1)), :);
        train_instance_matrix(12*i-4 : 12*i, :) = instance_matrix_jitter_mirror(5*trainid(((i-1)*40 + 1))-4 : 5*trainid(((i-1)*40 + 1)), :);

        train_label_vector(12*i-11 : 12*i) = image_labels(trainid(((i-1)*40 + 1)));
    end

elseif train_jitter == 1
    train_instance_matrix = zeros(17 * 6, 4096);
    train_label_vector = zeros(17 * 6, 1);
    for i = 1 : 17
        train_instance_matrix(6*i - 5, :) = instance_matrix_standard(trainid((i-1)*40 + 1), :);
        train_instance_matrix(6*i-4 : 6*i, :) = instance_matrix_jitter(5*(trainid((i-1)*40 + 1))-4 : 5*(trainid((i-1)*40 + 1)), :);
        train_label_vector(6*i-5 : 6*i) = image_labels(trainid((i-1)*40 + 1));
    end

elseif train_mirror == 1
    train_instance_matrix = zeros(17 * 2, 4096);
    train_label_vector = zeros(17 * 2, 1);
    for i = 1 : 17
        train_instance_matrix(2*i - 1, :) = instance_matrix_standard(trainid((i-1)*40 + 1), :);
        train_instance_matrix(2*i, :) = instance_matrix_mirror(trainid((i-1)*40 + 1), :);

        train_label_vector(2*i-1 : 2*i) = image_labels(trainid((i-1)*40 + 1));
    end

else
    train_instance_matrix = zeros(17, 4096);
    train_label_vector = zeros(17, 1);
    for i = 1 : 17
        train_instance_matrix(i, :) = instance_matrix_standard(trainid((i-1)*40 + 1), :);

        train_label_vector(i) = image_labels(trainid((i-1)*40 + 1));
    end
end
